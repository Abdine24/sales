...
export type NavPage = 'dashboard' | 'pos' | 'stock' | 'clients' | 'fournisseurs' | 'categories' | 'personnel' | 'settings' | 'sync';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className={`${collapsed ? 'w-[76px]' : 'w-64'} relative h-screen flex flex-col glass-panel border-r border-slate-200/50 dark:border-white/10 p-3 shrink-0 select-none z-20 transition-[width] duration-200`}>
      {/* Brand Header */}
      <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'} px-2 py-3 mb-4`}>
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-slate-900 via-blue-900 to-blue-600 dark:from-blue-600 dark:to-indigo-500 flex items-center justify-center text-white shadow-lg shadow-blue-500/20">
          <Apple className="w-6 h-6" />
        </div>
        <div className={collapsed ? 'hidden' : ''}>
          <h1 className="font-bold text-lg text-slate-900 dark:text-white tracking-tight leading-tight">
            iVente <span className="text-blue-500 text-xs font-semibold px-1.5 py-0.5 rounded-full bg-blue-500/10">Pro</span>
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-40">{settings?.slogan || 'Offline-First POS'}</p>
        </div>
      </div>
      ...
      {/* Footer Info */}
      <div className={`${collapsed ? 'hidden' : 'flex'} mt-auto pt-4 border-t border-slate-200/50 dark:border-white/10 px-2 text-xs text-slate-500 dark:text-slate-400 flex-col gap-1`}>
